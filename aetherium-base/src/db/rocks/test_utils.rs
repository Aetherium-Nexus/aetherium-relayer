...
mod test {
    use aetherium_core::{
        AetheriumDomain, AetheriumLogStore, AetheriumMessage, Indexed, LogMeta,
        RawAetheriumMessage, H256, H512, U256,
    };

    use crate::db::AetheriumRocksDB;
    ...
    async fn db_stores_and_retrieves_messages() {
        run_test_db(|db| async move {
            let db = AetheriumRocksDB::new(
                &AetheriumDomain::new_test_domain("db_stores_and_retrieves_messages"),
                db,
            );

            let m = AetheriumMessage {
                nonce: 100,
                version: 3,
                origin: 10,
            ...
            assert_eq!(
                RawAetheriumMessage::from(&by_nonce),
                RawAetheriumMessage::from(&m)
            );
        })
        .await;
    }
