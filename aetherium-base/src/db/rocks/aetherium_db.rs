...
use aetherium_core::{
    Decode, Encode, GasPaymentKey, AetheriumDomain, AetheriumLogStore, AetheriumMessage,
    AetheriumSequenceAwareIndexerStoreReader, AetheriumWatermarkedLogStore, Indexed,
...
#[derive(Debug, Clone)]
pub struct AetheriumRocksDB(AetheriumDomain, TypedDB);

impl std::ops::Deref for AetheriumRocksDB {
    type Target = TypedDB;
...
impl AsRef<TypedDB> for AetheriumRocksDB {
    fn as_ref(&self) -> &TypedDB {
        &self.1
    }
}

impl AsRef<DB> for AetheriumRocksDB {
    fn as_ref(&self) -> &DB {
        self.1.as_ref()
    }
}

impl AetheriumRocksDB {
    /// Instantiated new `AetheriumRocksDB`
    pub fn new(domain: &AetheriumDomain, db: DB) -> Self {
        Self(domain.clone(), TypedDB::new(domain, db))
    }

    /// Get the domain this database is scoped to
    pub fn domain(&self) -> &AetheriumDomain {
        &self.0
    }
    ...
    pub fn retrieve_message_by_nonce(&self, nonce: u32) -> DbResult<Option<AetheriumMessage>> {
...
#[async_trait]
impl AetheriumLogStore<AetheriumMessage> for AetheriumRocksDB {
...
async fn store_and_count_new<T: Copy>(
    store: &AetheriumRocksDB,
    logs: &[(T, LogMeta)],
    log_type: &str,
    process: impl Fn(&AetheriumRocksDB, T, &LogMeta) -> DbResult<bool>,
) -> Result<u32> {
    let mut new_logs = 0;
...
#[async_trait]
impl AetheriumLogStore<InterchainGasPayment> for AetheriumRocksDB {
    ...
    async fn store_logs(
        ...
        store_and_count_new(
            ...
            payments,
            "gas payments",
            AetheriumRocksDB::process_indexed_gas_payment,
        )
        .await
    }
}
...
#[async_trait]
impl AetheriumLogStore<MerkleTreeInsertion> for AetheriumRocksDB {
...
#[async_trait]
impl AetheriumSequenceAwareIndexerStoreReader<AetheriumMessage> for AetheriumRocksDB {
...
#[async_trait]
impl AetheriumSequenceAwareIndexerStoreReader<MerkleTreeInsertion> for AetheriumRocksDB {
    /// Gets data by its sequence.
...
#[async_trait]
impl AetheriumSequenceAwareIndexerStoreReader<InterchainGasPayment> for AetheriumRocksDB {
    /// Gets data by its sequence.
...
#[async_trait]
impl AetheriumWatermarkedLogStore<InterchainGasPayment> for AetheriumRocksDB {
...
#[async_trait]
impl AetheriumWatermarkedLogStore<AetheriumMessage> for AetheriumRocksDB {
    /// Gets the block number high watermark
    async fn retrieve_high_watermark(&self) -> Result<Option<u32>> {
...
// Keep this implementation for type compatibility with the `contract_syncs` sync builder
#[async_trait]
impl AetheriumWatermarkedLogStore<MerkleTreeInsertion> for AetheriumRocksDB {
    /// Gets the block number high watermark
    async fn retrieve_high_watermark(&self) -> Result<Option<u32>> {
        bail!("Not implemented")
    }
...
impl AetheriumDb for AetheriumRocksDB {
