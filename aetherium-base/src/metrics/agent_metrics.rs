...
use eyre::Result;
use aetherium_core::metrics::agent::decimals_by_protocol;
use aetherium_core::metrics::agent::u256_as_scaled_f64;
use aetherium_core::metrics::agent::METRICS_SCRAPE_INTERVAL;
use aetherium_core::AetheriumDomain;
use aetherium_core::AetheriumProvider;
use maplit::hashmap;
...
pub struct ChainSpecificMetricsUpdater {
    ...
    provider: Box<dyn AetheriumProvider>,
}
...
impl ChainSpecificMetricsUpdater {
    ...
    async fn update_block_details(&self) {
        if let AetheriumDomain::Unknown { .. } = self.conf.domain {
            return;
        };
        let chain = self.conf.domain.name();
