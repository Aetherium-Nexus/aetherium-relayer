//! Common settings and configuration for Aetherium agents
//!
...
//! ## Introduction
//!
//! Aetherium Agents have a shared core, which contains connection info for rpc,
...
//! with `AET_` will be read as an override to be applied against the hierarchical structure
...
//! `AET_CHAINS_TEST2_DOMAINID=1`, then the config parser will directly override the value of
...
//! 3. Configuration env vars with the prefix `AET` intended
//!    to be shared by multiple agents in the same environment
//!    E.g. `export AET_CHAINS_ARBITRUM_DOMAINID=3000`
...
mod envs {
    pub use aetherium_cosmos as h_cosmos;
    pub use aetherium_ethereum as h_eth;
    pub use aetherium_fuel as h_fuel;
    pub use aetherium_sealevel as h_sealevel;
}
...
macro_rules! impl_loadable_from_settings {
    ($agent:ident, $settingsparser:ident -> $settingsobj:ident) => {
        impl aetherium_base::LoadableFromSettings for $settingsobj {
            fn load() -> aetherium_core::config::ConfigResult<Self> {
                aetherium_base::settings::loader::load_settings::<$settingsparser, Self>()
            }
        }
    };
}
