...
use aetherium_core::{
    cfg_unwrap_all, config::*, AetheriumDomain, AetheriumDomainProtocol,
    AetheriumDomainTechnicalStack, IndexMode, ReorgPeriod,
};
        ...
        .unwrap_or_else(|| {
            domain
                ...
                .and_then(|d| match d.domain_protocol() {
                    AetheriumDomainProtocol::Ethereum => Some(IndexMode::Block),
                    AetheriumDomainProtocol::Sealevel => Some(IndexMode::Sequence),
                    _ => None,
                })
                .unwrap_or_default()
        });
...
fn parse_domain(chain: ValueParser, name: &str) -> ConfigResult<AetheriumDomain> {
    let mut err = ConfigParsingError::default();
    ...
    let protocol = chain
        .chain(&mut err)
        .get_key("protocol")
        .parse_from_str::<AetheriumDomainProtocol>("Invalid Aetherium domain protocol")
        .end();

    let technical_stack = chain
        .chain(&mut err)
        .get_opt_key("technicalStack")
        .parse_from_str::<AetheriumDomainTechnicalStack>("Invalid chain technical stack")
        .end()
        .or_else(|| Some(AetheriumDomainTechnicalStack::default()));

    cfg_unwrap_all!(&chain.cwp, err: [domain_id, protocol, technical_stack]);

    let domain = AetheriumDomain::from_config(domain_id, name, protocol, technical_stack)
