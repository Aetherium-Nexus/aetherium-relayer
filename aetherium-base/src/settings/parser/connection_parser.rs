...
use aetherium_core::config::{ConfigErrResultExt, OperationBatchConfig};
use aetherium_core::{config::ConfigParsingError, AetheriumDomainProtocol, NativeToken};
...
pub fn build_connection_conf(
    ...
    match domain_protocol {
        AetheriumDomainProtocol::Ethereum => build_ethereum_connection_conf(
            rpcs,
            chain,
            err,
        ...
        AetheriumDomainProtocol::Fuel => rpcs
            .iter()
            .next()
            .map(|url| ChainConnectionConf::Fuel(h_fuel::ConnectionConf { url: url.clone() })),
        AetheriumDomainProtocol::Sealevel => rpcs
            .iter()
            .next()
