...
use aetherium_core::{
    ...
    AetheriumAbi, AetheriumDomain, AetheriumDomainProtocol, AetheriumMessage, AetheriumProvider,
...
pub type MessageIndexer = Arc<dyn SequenceAwareIndexer<AetheriumMessage>>;
...
impl ChainConnectionConf {
    /// Get what aetherium protocol is in use for this chain.
    pub fn protocol(&self) -> AetheriumDomainProtocol {
        match self {
            Self::Ethereum(_) => AetheriumDomainProtocol::Ethereum,
            Self::Fuel(_) => AetheriumDomainProtocol::Fuel,
            Self::Sealevel(_) => AetheriumDomainProtocol::Sealevel,
            Self::Cosmos(_) => AetheriumDomainProtocol::Cosmos,
        }
    }
...
impl ChainConf {
    ...
    /// Try to convert the chain settings into an AetheriumProvider.
    pub async fn build_provider(
        &self,
        metrics: &CoreMetrics,
    ) -> Result<Box<dyn AetheriumProvider>> {
        let ctx = "Building provider";
        let locator = self.locator(H256::zero());
        match &self.connection {
            ChainConnectionConf::Ethereum(conf) => {
                self.build_ethereum(conf, &locator, metrics, h_eth::AetheriumProviderBuilder {})
                    .await
            }
            ChainConnectionConf::Fuel(_) => todo!(),
            ChainConnectionConf::Sealevel(conf) => {
                ...
                Ok(Box::new(provider) as Box<dyn AetheriumProvider>)
            }
    ...
    pub async fn build_mailbox(&self, metrics: &CoreMetrics) -> Result<Box<dyn Mailbox>> {
        ...
        match &self.connection {
            ...
            ChainConnectionConf::Fuel(conf) => {
                ...
                aetherium_fuel::FuelMailbox::new(conf, locator, wallet)
                    .await
    ...
    pub async fn build_message_indexer(
        ...
        metrics: &CoreMetrics,
        advanced_log_meta: bool,
    ) -> Result<Box<dyn SequenceAwareIndexer<AetheriumMessage>>> {
        ...
        match &self.connection {
            ...
            ChainConnectionConf::Sealevel(conf) => {
                ...
                Ok(indexer as Box<dyn SequenceAwareIndexer<AetheriumMessage>>)
            }
            ChainConnectionConf::Cosmos(conf) => {
                let signer = self.cosmos_signer().await.context(ctx)?;
