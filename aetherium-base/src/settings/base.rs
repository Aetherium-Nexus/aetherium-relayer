...
use aetherium_core::{
    AetheriumDomain, AetheriumLogStore, AetheriumProvider,
    AetheriumSequenceAwareIndexerStoreReader, AetheriumWatermarkedLogStore, InterchainGasPaymaster,
...
use crate::{
    ...
    ContractSync, ContractSyncMetrics, ContractSyncer, CoreMetrics, AetheriumAgentCore,
...
impl Settings {
    ...
    pub fn build_aetherium_core(&self, metrics: Arc<CoreMetrics>) -> AetheriumAgentCore {
        AetheriumAgentCore {
            metrics,
            settings: self.clone(),
        }
    ...
    pub fn chain_setup(&self, domain: &AetheriumDomain) -> Result<&ChainConf> {
        self.chains
            .get(domain.name())
    ...
    pub fn lookup_domain(&self, chain_name: &str) -> Result<AetheriumDomain> {
        self.chains
            .get(chain_name)
...
macro_rules! build_chain_conf_fns {
    ($singular:ident, $plural:ident -> $ret:ty) => {
        ...
        pub async fn $plural(
            &self,
            domains: impl Iterator<Item = &AetheriumDomain>,
            metrics: &CoreMetrics,
        ) -> HashMap<AetheriumDomain, eyre::Result<Arc<$ret>>> {
...
impl Settings {
    ...
    build_chain_conf_fns!(build_provider, build_providers -> dyn AetheriumProvider);
    ...
    where
        ...
        S: AetheriumLogStore<T> + AetheriumSequenceAwareIndexerStoreReader<T> + 'static,
    {
        let setup = self.chain_setup(domain)?;
    ...
    where
        T: Indexable + Debug,
        SequenceIndexer<T>: TryFromWithMetrics<ChainConf>,
        S: AetheriumLogStore<T> + AetheriumWatermarkedLogStore<T> + 'static,
    {
        let setup = self.chain_setup(domain)?;
        // Currently, all indexers are of the `SequenceIndexer` type
        let indexer =
    ...
    pub async fn contract_syncs<T, S>(
        ...
        metrics: &CoreMetrics,
        sync_metrics: &ContractSyncMetrics,
        stores: HashMap<AetheriumDomain, Arc<S>>,
        advanced_log_meta: bool,
    ) -> Result<HashMap<AetheriumDomain, Arc<dyn ContractSyncer<T>>>>
    where
        T: Indexable + Debug + Send + Sync + Clone + Eq + Hash + 'static,
        SequenceIndexer<T>: TryFromWithMetrics<ChainConf>,
        S: AetheriumLogStore<T>
            + AetheriumSequenceAwareIndexerStoreReader<T>
            + AetheriumWatermarkedLogStore<T>
            + 'static,
    {
