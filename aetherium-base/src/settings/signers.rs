...
use aetherium_core::{AccountAddressType, H256};
use aetherium_sealevel::Keypair;
...
#[async_trait]
impl BuildableWithSignerConf for aetherium_ethereum::Signers {
    async fn build(conf: &SignerConf) -> Result<Self, Report> {
        Ok(match conf {
            SignerConf::HexKey { key } => aetherium_ethereum::Signers::Local(LocalWallet::from(
            ...
            SignerConf::Aws { id, region } => {
                ...
                aetherium_ethereum::Signers::Aws(signer)
            }
...
impl ChainSigner for aetherium_ethereum::Signers {
    fn address_string(&self) -> String {
...
#[async_trait]
impl BuildableWithSignerConf for aetherium_cosmos::Signer {
    async fn build(conf: &SignerConf) -> Result<Self, Report> {
        ...
        {
            Ok(aetherium_cosmos::Signer::new(
                key.as_bytes().to_vec(),
                prefix.clone(),
                account_address_type,
            )?)
...
impl ChainSigner for aetherium_cosmos::Signer {
    fn address_string(&self) -> String {
        self.address.clone()
    }
}
