    ...
    R: FromRawConf<T>,
{
    ...
    let config_deserializer = builder
        ...
        .add_source(CaseAdapter::new(
            Environment::default().prefix("AET_").separator("_"),
            Case::Flat,
        ))
        .add_source(CaseAdapter::new(
    ...
    let raw_config = Config::try_deserialize::<T>(config_deserializer)
        .or_else(|err| {
            ...
            err.context("Config deserialization error, please check the config reference (https://docs.aetherium-nexus.com/docs/operators/agent-configuration/configuration-reference)")
        })
        .into_config_result(|| root_path.clone())?;
