...
use eyre::Result;
use aetherium_core::{
    utils::fmt_sync_time, ContractSyncCursor, CursorAction, AetheriumDomain, AetheriumLogStore,
    AetheriumSequenceAwareIndexerStore, AetheriumWatermarkedLogStore, Indexer,
    SequenceAwareIndexer,
};
use aetherium_core::{Indexed, LogMeta, H512};
...
#[derive(Debug)]
pub struct ContractSync<T: Indexable, S: AetheriumLogStore<T>, I: Indexer<T>> {
...
impl<T: Indexable, S: AetheriumLogStore<T>, I: Indexer<T>> ContractSync<T, S, I> {
...
pub type WatermarkLogStore<T> = Arc<dyn AetheriumWatermarkedLogStore<T>>;

/// A ContractSync for syncing events using a RateLimitedContractSyncCursor
pub type WatermarkContractSync<T> =
    SequenceAwareContractSync<T, Arc<dyn AetheriumWatermarkedLogStore<T>>>;
...
pub type SequenceAwareLogStore<T> = Arc<dyn AetheriumSequenceAwareIndexerStore<T>>;

/// A ContractSync for syncing messages using a SequenceSyncCursor
pub type SequencedDataContractSync<T> =
    SequenceAwareContractSync<T, Arc<dyn AetheriumSequenceAwareIndexerStore<T>>>;
