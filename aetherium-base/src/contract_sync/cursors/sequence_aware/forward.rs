...
use aetherium_core::{
    indexed_to_sequence_indexed_array, ContractSyncCursor, CursorAction, AetheriumDomain,
    AetheriumSequenceAwareIndexerStoreReader, IndexMode, Indexed, LogMeta, SequenceAwareIndexer,
    SequenceIndexed,
};
...
pub(crate) struct ForwardSequenceAwareSyncCursor<T> {
    ...
    store: Arc<dyn AetheriumSequenceAwareIndexerStoreReader<T>>,
...
pub(crate) mod test {
    use derive_new::new;
    use aetherium_core::{
        ChainResult, AetheriumDomainProtocol, AetheriumLogStore, Indexed, Indexer, Sequenced,
    };
    ...
    #[derive(Debug, Clone)]
    pub struct MockAetheriumSequenceAwareIndexerStore<T> {
        pub logs: Vec<(T, LogMeta)>,
    }

    #[async_trait]
    impl<T: Sequenced + Debug + Clone + Send + Sync + Indexable + 'static> AetheriumLogStore<T>
        for MockAetheriumSequenceAwareIndexerStore<T>
    {
    ...
    impl<T: Sequenced + Debug + Clone + Send + Sync + Indexable + 'static>
        AetheriumSequenceAwareIndexerStoreReader<T> for MockAetheriumSequenceAwareIndexerStore<T>
    {
    ...
    impl Indexable for MockSequencedData {
        fn indexing_cursor(_domain: AetheriumDomainProtocol) -> CursorType {
    ...
    async fn get_test_forward_sequence_aware_sync_cursor(
        ...
        let db = Arc::new(MockAetheriumSequenceAwareIndexerStore {
            logs: vec![
        ...
        let metrics_data = MetricsData {
            domain: AetheriumDomain::new_test_domain("test"),
