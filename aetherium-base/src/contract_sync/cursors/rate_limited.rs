...
use aetherium_core::{
    ContractSyncCursor, CursorAction, AetheriumDomain, AetheriumWatermarkedLogStore, Indexed,
    Indexer, LogMeta,
};
...
pub(crate) struct RateLimitedContractSyncCursor<T> {
    ...
    store: Arc<dyn AetheriumWatermarkedLogStore<T>>,
    tip: u32,
...
pub(crate) mod test {
    ...
    use aetherium_core::{ChainResult, AetheriumDomainProtocol, AetheriumLogStore};
    ...
    mockall::mock! {
        ...
        impl<T: Indexable + Send + Sync> Indexer<T> for Indexer<T> {
            async fn fetch_logs_in_range(&self, range: RangeInclusive<u32>) -> ChainResult<Vec<(aetherium_core::Indexed<T>, LogMeta)>>;
    ...
    mockall::mock! {
        ...
        #[async_trait]
        impl<T: Indexable + Send + Sync> AetheriumLogStore<T> for Db<T> {
            async fn store_logs(&self, logs: &[(aetherium_core::Indexed<T>, LogMeta)]) -> Result<u32>;
        }

        #[async_trait]
        impl<T: Indexable + Send + Sync> AetheriumWatermarkedLogStore<T> for Db<T> {
    ...
    async fn mock_rate_limited_cursor<T: Indexable + Debug + Send + Sync + 'static>(
        ...
        RateLimitedContractSyncCursor::new(
            Arc::new(indexer),
            Arc::new(metrics),
            &AetheriumDomain::new_test_domain("test"),
            Arc::new(db),
            chunk_size,
            initial_height,
        )
