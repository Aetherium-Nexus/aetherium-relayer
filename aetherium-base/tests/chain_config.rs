...
use eyre::Context;
use aetherium_base::settings::{parser::RawAgentConf, Settings};
use aetherium_core::{config::*, KnownAetheriumDomain};
use walkdir::WalkDir;

/// Relative path to the `aetherium-relayer/config/`
...
/// Provides a vector of parsed `aetherium_base::Settings` objects
...
fn aetherium_settings() -> Vec<Settings> {
    ...
    paths
        ...
        .filter_map(|(p, f)| {
            ...
            Settings::from_config(raw, &ConfigPath::default())
                .context("Config parsing error, please check the config reference (https://docs.aetherium-nexus.com/docs/operators/agent-configuration/configuration-reference)")
                .ok()
        })
        .collect()
}
...
fn agent_json_config_consistency_checks() {
    ...
    // mapping in `aetherium-core/src/chain.rs` named
    ...
    for ChainCoordinate { name, domain } in chain_coords.into_iter() {
        assert_eq!(
            KnownAetheriumDomain::try_from(domain).unwrap().to_string(),
            name
        );
        assert_eq!(name.parse::<KnownAetheriumDomain>().unwrap() as u32, domain);
    }
}
