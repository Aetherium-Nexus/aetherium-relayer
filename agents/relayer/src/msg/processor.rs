...
use eyre::Result;
use aetherium_base::{
    db::{AetheriumDb, AetheriumRocksDB},
    CoreMetrics,
};
use aetherium_core::{AetheriumDomain, AetheriumMessage, QueueOperation};
...
impl ForwardBackwardIterator {
    ...
    fn new(db: Arc<dyn AetheriumDb>) -> Self {
    ...
    async fn try_get_next_message(
        &mut self,
        metrics: &MessageProcessorMetrics,
    ) -> Result<Option<AetheriumMessage>> {
        loop {
...
impl DirectionalNonceIterator {
    ...
    fn try_get_next_nonce(
        &self,
        metrics: &MessageProcessorMetrics,
    ) -> Result<MessageStatus<AetheriumMessage>> {
        if let Some(message) = self.indexed_message_with_nonce()? {
            ...
            if !self.is_message_processed()? {
                debug!(aet_message=?message, iterator=?self, "Found processable message");
    ...
    fn update_max_nonce_gauge(message: &AetheriumMessage, metrics: &MessageProcessorMetrics) {
    ...
    fn indexed_message_with_nonce(&self) -> Result<Option<AetheriumMessage>> {
        match self.nonce {
            Some(nonce) => {
...
impl ProcessorExt for MessageProcessor {
    ...
    async fn tick(&mut self) -> Result<()> {
        // Forever, scan AetheriumRocksDB looking for new messages to send. When criteria are
...
impl MessageProcessor {
    ...
    pub fn new(
        ...
        Self {
            ...
            destination_ctxs,
            metric_app_contexts,
            nonce_iterator: ForwardBackwardIterator::new(Arc::new(db) as Arc<dyn AetheriumDb>),
            max_retries,
        }
    }

    async fn try_get_unprocessed_message(&mut self) -> Result<Option<AetheriumMessage>> {
...
impl MessageProcessorMetrics {
    pub fn new<'a>(
        metrics: &CoreMetrics,
        origin: &AetheriumDomain,
        destinations: impl Iterator<Item = &'a AetheriumDomain>,
    ) -> Self {
...
mod test {
    ...
    use aetherium_base::{
        db::{
            test_utils, DbResult, AetheriumRocksDB, InterchainGasExpenditureData,
    ...
    use aetherium_operation_verifier::{
        ApplicationOperationVerifier, ApplicationOperationVerifierReport,
    };
    use aetherium_test::mocks::{MockMailboxContract, MockValidatorAnnounceContract};
    ...
    fn dummy_chain_conf(domain: &AetheriumDomain) -> ChainConf {
        ChainConf {
            domain: domain.clone(),
            signer: Default::default(),
            reorg_period: Default::default(),
            addresses: Default::default(),
            connection: ChainConnectionConf::Ethereum(aetherium_ethereum::ConnectionConf {
                rpc_connection: aetherium_ethereum::RpcConnectionConf::Http {
    ...
    fn dummy_metadata_builder(
        origin_domain: &AetheriumDomain,
        destination_domain: &AetheriumDomain,
        db: &AetheriumRocksDB,
    ) -> BaseMetadataBuilder {
    ...
    fn dummy_aetherium_message(destination: &AetheriumDomain, nonce: u32) -> AetheriumMessage {
        AetheriumMessage {
    ...
    fn add_db_entry(db: &AetheriumRocksDB, msg: &AetheriumMessage, retry_count: u32) {
    ...
    fn persist_retried_messages(
        ...
        retries.iter().for_each(|num_retries| {
            let message = dummy_aetherium_message(destination_domain, nonce);
    ...
    mockall::mock! {
        ...
        impl AetheriumDb for Db {
            ...
            fn retrieve_message_by_nonce(&self, nonce: u32) -> DbResult<Option<AetheriumMessage>>;
            ...
            fn store_message_by_id(&self, id: &H256, message: &AetheriumMessage) -> DbResult<()>;

            fn retrieve_message_by_id(&self, id: &H256) -> DbResult<Option<AetheriumMessage>>;
    ...
    async fn test_full_pending_message_persistence_flow() {
        test_utils::run_test_db(|db| async move {
            ...
            let db = AetheriumRocksDB::new(&origin_domain, db);

            // Assume the message syncer stored some new messages in AetheriumDB
            ...
            // Set some retry counts. This should update AetheriumDB entries too.
            ...
            // Expect the AetheriumDB entry to have been updated, so the `OpQueue` in the submitter
        ...
        mock_db
            ...
            .returning(move |nonce| {
                ...
                if nonce > MAX_ONCHAIN_NONCE {
                    ...
                    Ok(Some(dummy_aetherium_message(
