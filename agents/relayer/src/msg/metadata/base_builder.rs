...
use aetherium_core::{
    accumulator::merkle::Proof, AggregationIsm, CcipReadIsm, Checkpoint, AetheriumDomain,
    AetheriumMessage, InterchainSecurityModule, MultisigIsm, RoutingIsm, ValidatorAnnounce, H160,
    H256,
};
...
pub trait BuildsBaseMetadata: Send + Sync + Debug {
    fn origin_domain(&self) -> &AetheriumDomain;
        ...
        for (&validator, validator_storage_locations) in validators.iter().zip(storage_locations) {
            ...
            if checkpoint_syncers.get(&validator.into()).is_none() {
                if validator_storage_locations.is_empty() {
                    warn!(?validator, "Validator has not announced any storage locations; see https://docs.aetherium-nexus.com/docs/operators/validators/announcing-your-validator");
                } else {
                    warn!(
                        ?validator,
