...
use aetherium_core::{utils::bytes_to_hex, AetheriumMessage, RawAetheriumMessage, H256};
use aetherium_ethereum::OffchainLookup;
...
impl MetadataBuilder for CcipReadIsmMetadataBuilder {
    ...
    async fn build(
        ...
        let response = ism
            .get_offchain_verify_info(RawAetheriumMessage::from(message).to_vec())
            .await;
