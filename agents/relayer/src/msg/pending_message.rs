...
use aetherium_base::{db::AetheriumDb, CoreMetrics};
use aetherium_core::{
    gas_used_by_operation, BatchItem, ChainCommunicationError, ChainResult, ConfirmReason,
    AetheriumChain, AetheriumDomain, AetheriumMessage, Mailbox, MessageSubmissionData,
...
pub struct MessageContext {
    ...
    pub origin_db: Arc<dyn AetheriumDb>,
...
impl TryBatchAs<AetheriumMessage> for PendingMessage {
    fn try_batch(&self) -> ChainResult<BatchItem<AetheriumMessage>> {
...
impl PendingOperation for PendingMessage {
    ...
    fn destination_domain(&self) -> &AetheriumDomain {
...
impl PendingMessage {
    /// Constructor that tries reading the retry count from the AetheriumDB in order to recompute the `next_attempt_after`.
    ...
    fn get_num_retries(origin_db: Arc<dyn AetheriumDb>, message: &AetheriumMessage) -> u32 {
        match origin_db.retrieve_pending_message_retry_count_by_message_id(&message.id()) {
            Ok(Some(num_retries)) => num_retries,
            r => {
                trace!(message_id = ?message.id(), result = ?r, "Failed to read retry count from AetheriumDB for message.");
                0
            }
        }
    }
    ...
    /// Record in AetheriumDB and various metrics that this process has observed
    ...
    /// `return Ok(())`, then without a wiped AetheriumDB, we will never
...
impl MessageSubmissionMetrics {
    ...
    fn update_nonce(&self, msg: &AetheriumMessage) {
