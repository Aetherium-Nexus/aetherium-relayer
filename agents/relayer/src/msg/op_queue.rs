...
use derive_new::new;
use aetherium_core::{PendingOperation, PendingOperationStatus, QueueOperation};
...
pub mod test {
    ...
    use super::*;
    use aetherium_core::{
        AetheriumDomain, AetheriumDomainProtocol, AetheriumDomainTechnicalStack,
        AetheriumDomainType, AetheriumMessage, KnownAetheriumDomain, PendingOperationResult,
    ...
    impl MockPendingOperation {
        pub fn new(seconds_to_next_attempt: u64, destination_domain: AetheriumDomain) -> Self {
            Self {
                id: H256::random(),
        ...
        pub fn with_message_data(message: AetheriumMessage) -> Self {
            Self {
                id: message.id(),
                ...
                retry_count: 0,
                destination_domain: AetheriumDomain::Unknown {
        ...
        pub fn with_origin_domain(self, domain: AetheriumDomain) -> Self {
            let domain_id = match domain {
                AetheriumDomain::Known(d) => d as u32,
    ...
    impl TryBatchAs<AetheriumMessage> for MockPendingOperation {}
    ...
    async fn test_multiple_op_queues_message_id() {
        ...
        let destination_domain: AetheriumDomain = KnownAetheriumDomain::Injective.into();
    ...
    async fn test_destination_domain() {
        ...
        // Add some operations to the queue with increasing `next_attempt_after` values
        let destination_domain_1: AetheriumDomain = KnownAetheriumDomain::Injective.into();
        let destination_domain_2: AetheriumDomain = KnownAetheriumDomain::Ethereum.into();
        let ops = vec![
        ...
        broadcaster
            .send(MessageRetryRequest {
                ...
                pattern: MatchingList::with_destination_domain(
                    KnownAetheriumDomain::Arbitrum as u32,
                ),
                transmitter: transmitter.clone(),
            })
            .unwrap();
    ...
    async fn test_process_retry_requests_multiple_fields() {
        ...
        let test_messages = [
            MockPendingOperation::new(10, KnownAetheriumDomain::Arbitrum.into()),
            MockPendingOperation::new(10, KnownAetheriumDomain::Optimism.into())
                .with_origin_domain(AetheriumDomain::Known(KnownAetheriumDomain::Optimism)),
        ];
        ...
        broadcaster
            .send(MessageRetryRequest {
                ...
                pattern: MatchingList(Some(vec![ListElement::new(
                    Filter::Wildcard,
                    Filter::Enumerated(vec![KnownAetheriumDomain::Optimism as u32]),
                    Filter::Wildcard,
                    Filter::Enumerated(vec![KnownAetheriumDomain::Optimism as u32]),
                    Filter::Wildcard,
                )])),
            ...
            .send(MessageRetryRequest {
                ...
                pattern: MatchingList(Some(vec![
                    ...
                    ListElement::new(
                        ...
                        Filter::Wildcard,
                        Filter::Wildcard,
                        Filter::Enumerated(vec![KnownAetheriumDomain::Arbitrum as u32]),
                        Filter::Wildcard,
                    ),
                ])),
                transmitter: transmitter.clone(),
