...
use aetherium_core::{cfg_unwrap_all, config::*, AetheriumDomain, U256};
...
pub struct RelayerSettings {
    ...
    pub origin_chains: HashSet<AetheriumDomain>,
    /// Chains to relay messages to
    pub destination_chains: HashSet<AetheriumDomain>,
...
impl FromRawConf<RawRelayerSettings> for RelayerSettings {
    fn from_config_filtered(
        ...
        let db = p
            ...
            .unwrap_or_else(|| std::env::current_dir().unwrap().join("aetherium_db"));
        ...
        let relay_chains: HashSet<AetheriumDomain> = relay_chain_names
            .unwrap_or_default()
            .into_iter()
            .filter_map(|chain| {
                base.lookup_domain(chain)
