...
use aetherium_base::{
    ...
    AetheriumAgentCore, RuntimeMetrics, SyncOptions,
};
use aetherium_core::{
    rpc_clients::call_and_retry_n_times, ChainCommunicationError, ContractSyncCursor,
    AetheriumDomain, AetheriumMessage, InterchainGasPayment, Mailbox, MerkleTreeInsertion,
    QueueOperation, ValidatorAnnounce, H512, U256,
};
use aetherium_operation_verifier::ApplicationOperationVerifier;
...
#[derive(AsRef)]
pub struct Relayer {
    origin_chains: HashSet<AetheriumDomain>,
    destination_chains: HashMap<AetheriumDomain, ChainConf>,
    #[as_ref]
    core: AetheriumAgentCore,
    message_syncs: HashMap<AetheriumDomain, Arc<dyn ContractSyncer<AetheriumMessage>>>,
    interchain_gas_payment_syncs:
        HashMap<AetheriumDomain, Arc<dyn ContractSyncer<InterchainGasPayment>>>,
    /// Context data for each (origin, destination) chain pair a message can be
    /// sent between
    msg_ctxs: HashMap<ContextKey, Arc<MessageContext>>,
    prover_syncs: HashMap<AetheriumDomain, Arc<RwLock<MerkleTreeBuilder>>>,
    merkle_tree_hook_syncs: HashMap<AetheriumDomain, Arc<dyn ContractSyncer<MerkleTreeInsertion>>>,
    dbs: HashMap<AetheriumDomain, AetheriumRocksDB>,
...
impl BaseAgent for Relayer {
    ...
    where
        Self: Sized,
    {
        let core = settings.build_aetherium_core(core_metrics.clone());
        let db = DB::from_path(&settings.db)?;
        let dbs = settings
            .origin_chains
            .iter()
            .map(|origin| (origin.clone(), AetheriumRocksDB::new(origin, db.clone())))
            .collect::<HashMap<_, _>>();
        ...
        let message_syncs: HashMap<_, Arc<dyn ContractSyncer<AetheriumMessage>>> = settings
            .contract_syncs::<AetheriumMessage, _>(
...
impl Relayer {
    ...
    pub async fn build_mailboxes(
        ...
        chain_metrics: &ChainMetrics,
    ) -> HashMap<AetheriumDomain, Arc<dyn Mailbox>> {
        settings
    ...
    pub async fn build_validator_announces(
        ...
        core_metrics: &CoreMetrics,
        chain_metrics: &ChainMetrics,
    ) -> HashMap<AetheriumDomain, Arc<dyn ValidatorAnnounce>> {
        settings
    ...
    pub async fn build_application_operation_verifiers(
        ...
        core_metrics: &CoreMetrics,
        chain_metrics: &ChainMetrics,
    ) -> HashMap<AetheriumDomain, Arc<dyn ApplicationOperationVerifier>> {
        settings
...
mod test {
    ...
    use aetherium_core::{
        config::OperationBatchConfig, AetheriumDomain, IndexMode, KnownAetheriumDomain,
        ReorgPeriod, H256,
    };
    use aetherium_ethereum as h_eth;
    ...
    fn generate_test_relayer_settings() -> RelayerSettings {
        let chains = [(
            "arbitrum".to_string(),
            ChainConf {
                domain: AetheriumDomain::Known(KnownAetheriumDomain::Arbitrum),
                signer: None,
        ...
        RelayerSettings {
            ...
            db: PathBuf::new(),
            origin_chains: [
                AetheriumDomain::Known(KnownAetheriumDomain::Arbitrum),
                AetheriumDomain::Known(KnownAetheriumDomain::Ethereum),
                AetheriumDomain::Known(KnownAetheriumDomain::Optimism),
            ]
            .into_iter()
            .collect(),
            destination_chains: [
    ...
    async fn test_failed_build_mailboxes() {
        ...
        assert!(mailboxes.contains_key(&AetheriumDomain::Known(KnownAetheriumDomain::Arbitrum)));
