...
use aetherium_base::db::AetheriumDb;
use aetherium_base::{CheckpointSyncer, CoreMetrics};
...
use aetherium_core::{
    ...
    AetheriumChain, AetheriumContract, AetheriumDomain, AetheriumSignerExt,
};
use aetherium_core::{ChainResult, MerkleTreeHook, ReorgEvent, ReorgPeriod};
use aetherium_ethereum::SingletonSignerHandle;
...
impl ValidatorSubmitterMetrics {
    pub fn new(metrics: &CoreMetrics, mailbox_chain: &AetheriumDomain) -> Self {
...
mod test {
    ...
    use eyre::Result;
    use aetherium_base::{
        db::{DbResult, AetheriumDb, InterchainGasExpenditureData, InterchainGasPaymentData},
        AgentMetadata,
    };
    use aetherium_core::{
        test_utils::dummy_domain, GasPaymentKey, AetheriumChain, AetheriumContract,
        AetheriumDomain, AetheriumMessage, AetheriumProvider, InterchainGasPayment,
    ...
    mockall::mock! {
        ...
        impl AetheriumChain for MerkleTreeHook {
            fn domain(&self) -> &AetheriumDomain;
            fn provider(&self) -> Box<dyn AetheriumProvider>;
        }

        impl AetheriumContract for MerkleTreeHook {
