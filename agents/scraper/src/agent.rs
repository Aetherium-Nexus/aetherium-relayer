...
use aetherium_core::{Delivery, AetheriumDomain, AetheriumMessage, InterchainGasPayment, H512};
...
use aetherium_base::{
    ...
    CoreMetrics, AetheriumAgentCore, RuntimeMetrics, SyncOptions,
};

use crate::{db::ScraperDb, settings::ScraperSettings, store::AetheriumDbStore};
...
impl BaseAgent for Scraper {
        ...
        Self: Sized,
    {
        ...
        let core = settings.build_aetherium_core(metrics.clone());
...
impl Scraper {
    ...
    async fn build_chain_scraper(
        ...
        info!(domain = domain.name(), "create AetheriumProvider");
        let provider = settings
            .build_provider(domain, &metrics.clone())
            .await?
            .into();
        info!(domain = domain.name(), "create AetheriumDbStore");
        let store = AetheriumDbStore::new(
            scraper_db,
            domain.clone(),
    ...
    async fn build_message_indexer(
        ...
        let sync = self
            .as_ref()
            .settings
            .sequenced_contract_sync::<AetheriumMessage, _>(
                &domain,
                &metrics.clone(),
...
mod test {
    ...
    use aetherium_core::{
        config::OperationBatchConfig, IndexMode, KnownAetheriumDomain, ReorgPeriod, H256,
    };
    ...
    async fn test_failed_build_chain_scrapers() {
        ...
        assert!(scrapers.contains_key(&(KnownAetheriumDomain::Arbitrum as u32)));
