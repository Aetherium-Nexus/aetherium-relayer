...
# Replaces https://docs.aetherium-nexus.com/docs/operate/set-up-agent-keys
module "iam_kms" {
  source = "../iam_kms"
...
# Replaces https://docs.aetherium-nexus.com/docs/operate/validators/validator-aws
module "s3" {
  source = "../s3"
...
resource "aws_ecs_task_definition" "validator" {
  ...
  container_definitions = jsonencode([
    {
      name  = "validator",
      image = "gcr.io/abacus-labs-dev/aetherium-agent:${var.validator_image_version}",
      user  = "1000:1000",
      secrets = [
        {
      ...
      mountPoints = [
        {
          sourceVolume  = "aetherium_db",
          containerPath = "/aetherium_db"
        },
      ],
      portMappings = [
        {
