...
use crate::{KnownAetheriumDomain, H160, H256, U256};
...
/// Computes hash of domain concatenated with "AETHERIUM"
...
/// Computes hash of domain concatenated with "AETHERIUM_ANNOUNCEMENT"
pub fn announcement_domain_hash(address: H256, domain: impl Into<u32>) -> H256 {
    H256::from_slice(
        Keccak256::new()
            .chain(domain.into().to_be_bytes())
            .chain(address)
            .chain("AETHERIUM_ANNOUNCEMENT")
            .finalize()
            .as_slice(),
    )
}
...
pub fn fmt_address_for_domain(domain: u32, addr: H256) -> String {
    KnownAetheriumDomain::try_from(domain)
