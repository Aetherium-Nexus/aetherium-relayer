...
#[error(transparent)]
pub struct AetheriumSignerError(#[from] Box<dyn std::error::Error + Send + Sync>);

/// A aetherium signer for use by the validators. Currently signers will always
/// use ethereum wallets.
#[async_trait]
#[auto_impl(&, Box, Arc)]
pub trait AetheriumSigner: Send + Sync + Debug {
    /// The signer's address
    fn eth_address(&self) -> H160;

    /// Sign a aetherium checkpoint hash. This must be a signature without eip
    /// 155.
    async fn sign_hash(&self, hash: &H256) -> Result<Signature, AetheriumSignerError>;
}

/// Auto-implemented extension trait for AetheriumSigner.
#[async_trait]
pub trait AetheriumSignerExt {
    /// Sign a `Signable` value
    async fn sign<T: Signable + Send>(
        &self,
        value: T,
    ) -> Result<SignedType<T>, AetheriumSignerError>;
    ...
    fn verify<T: Signable>(
        &self,
        signed: &SignedType<T>,
    ) -> Result<(), crate::AetheriumProtocolError>;
}

#[async_trait]
impl<S: AetheriumSigner> AetheriumSignerExt for S {
    async fn sign<T: Signable + Send>(
        &self,
        value: T,
    ) -> Result<SignedType<T>, AetheriumSignerError> {
...
impl<T: Signable> SignedType<T> {
    ...
    #[cfg(feature = "ethers")]
    pub fn recover(&self) -> Result<H160, crate::AetheriumProtocolError> {
    ...
    /// Check whether a message was signed by a specific address
    #[cfg(feature = "ethers")]
    pub fn verify(&self, signer: H160) -> Result<(), crate::AetheriumProtocolError> {
        let hash = ethers_core::types::H256::from(self.value.eth_signed_message_hash());
        let sig = ethers_core::types::Signature::from(self.signature);
...
mod hashes {
    ...
    fn ensure_signed_hashes_match() {
        ...
        assert_eq!(
            ethers_core::utils::hash_message(b"aetherium"),
            hash_message(b"aetherium").into()
        );
    }
}
