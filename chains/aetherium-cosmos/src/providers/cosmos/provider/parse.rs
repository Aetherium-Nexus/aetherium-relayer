...
impl TryFrom<&Any> for PacketData {
    ...
    fn try_from(any: &Any) -> Result<Self, Self::Error> {
        ...
        let msg = MsgRecvPacket::decode(vec).map_err(Into::<AetheriumCosmosError>::into)?;
        let packet = msg
            .packet
            .ok_or(AetheriumCosmosError::UnparsableEmptyField(
...
mod tests {
    ...
    fn fail_json() {
        ...
        assert!(matches!(
            data.err().unwrap(),
            AetheriumCosmosError::SerdeError(_),
        ));
    }
    ...
    fn fail_empty() {
        ...
        assert!(matches!(
            data.err().unwrap(),
            AetheriumCosmosError::UnparsableEmptyField(_),
        ));
    }
