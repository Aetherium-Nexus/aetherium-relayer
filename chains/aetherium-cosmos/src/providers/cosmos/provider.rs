...
use crate::{
    ConnectionConf, CosmosAccountId, CosmosAddress, CosmosAmount, AetheriumCosmosError, Signer,
};
...
impl CosmosProvider {
    ...
    pub fn new(
        ...
        chain: Option<aetherium_metric::prometheus_metric::ChainInfo>,
    ) -> ChainResult<Self> {
    ...
    fn convert_signer_info_into_account_id_and_nonce(
        ...
        let signer_public_key = signer_info.public_key.clone().ok_or_else(|| {
            AetheriumCosmosError::PublicKeyError("no public key for default signer".to_owned())
        })?;
        ...
        let public_key_and_account_address_type = match signer_public_key {
            ...
            SignerPublicKey::Any(pk) => {
                    ...
                    && pk.type_url != INJECTIVE_PUBLIC_KEY_TYPE_URL
                {
                    ...
                    Err(AetheriumCosmosError::PublicKeyError(msg.to_owned()))?
                }
                ...
                let (pub_key, account_address_type) =
                    if pk.type_url == INJECTIVE_PUBLIC_KEY_TYPE_URL {
                        ...
                        let proto: proto::cosmos::crypto::secp256k1::PubKey =
                            any.to_msg().map_err(Into::<AetheriumCosmosError>::into)?;

                        let decompressed = decompress_public_key(&proto.key)
                            .map_err(|e| AetheriumCosmosError::PublicKeyError(e.to_string()))?;

                        let tendermint = tendermint::PublicKey::from_raw_secp256k1(&decompressed)
                            .ok_or_else(|| {
                            AetheriumCosmosError::PublicKeyError(
            ...
            .map_or_else(
                || {
                    ...
                    let signer_info = tx.auth_info.signer_infos.get(0).ok_or_else(|| {
                        AetheriumCosmosError::SignerInfoError(
            ...
            Err(msg_execute_contract_error) => {
                match Self::contract_address_from_msg_recv_packet(tx) {
                    ...
                    Err(msg_recv_packet_error) => {
                        ...
                        let error = AetheriumCosmosError::ParsingAttemptsFailed(errors);
                        warn!(?tx_hash, ?error);
    ...
    fn contract_address_from_msg_execute_contract(tx: &Tx) -> Result<H256, AetheriumCosmosError> {
    ...
    fn contract_address_from_msg_recv_packet(tx: &Tx) -> Result<H256, AetheriumCosmosError> {
        let packet_data = tx
            .body
            .messages
            .iter()
...
impl AetheriumChain for CosmosProvider {
...
#[async_trait]
impl AetheriumProvider for CosmosProvider {
