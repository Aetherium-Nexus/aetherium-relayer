...
use aetherium_core::config::OperationBatchConfig;
use aetherium_core::{ContractLocator, AetheriumDomain, KnownAetheriumDomain, NativeToken};
...
fn provider(address: &str) -> WasmGrpcProvider {
    let domain = AetheriumDomain::Known(KnownAetheriumDomain::Neutron);
