...
use aetherium_core::{
    ChainCommunicationError, ChainResult, ContractLocator, Decode, AetheriumMessage, Indexed,
...
use crate::{ConnectionConf, CosmosMailbox, AetheriumCosmosError, Signer};
...
impl CosmosMailboxDispatchIndexer {
    ...
    #[instrument(err)]
    fn aetherium_message_parser(
        attrs: &Vec<EventAttribute>,
    ) -> ChainResult<ParsedEvent<AetheriumMessage>> {
        let mut contract_address: Option<String> = None;
        let mut message: Option<AetheriumMessage> = None;
                    ...
                    match key {
                        ...
                        v if *CONTRACT_ADDRESS_ATTRIBUTE_KEY_BASE64 == v => {
                            contract_address = Some(String::from_utf8(
                                BASE64
                                    .decode(value)
                                    .map_err(Into::<AetheriumCosmosError>::into)?,
                            )?);
                        }
                        ...
                        MESSAGE_ATTRIBUTE_KEY => {
                            ...
                            message = Some(AetheriumMessage::read_from(&mut reader)?);
                        }
...
#[async_trait]
impl Indexer<AetheriumMessage> for CosmosMailboxDispatchIndexer {
    async fn fetch_logs_in_range(
        ...
        let logs_futures = parse_logs_in_range(
            range,
            self.provider.clone(),
            Self::aetherium_message_parser,
            "AetheriumMessageCursor",
        );
...
#[async_trait]
impl SequenceAwareIndexer<AetheriumMessage> for CosmosMailboxDispatchIndexer {
...
mod tests {
    ...
    #[test]
    fn test_aetherium_message_parser() {
        ...
        let expected = ParsedEvent::new(
            ...
            AetheriumMessage::from(hex::decode("").unwrap()),
        );

        let assert_parsed_event = |attrs: &Vec<EventAttribute>| {
            let parsed_event =
                CosmosMailboxDispatchIndexer::aetherium_message_parser(attrs).unwrap();
