...
use aetherium_core::{ChainCommunicationError, ChainResult, H256, H512};

use crate::error::AetheriumSealevelError;

pub fn from_base58(base58: &str) -> Result<Vec<u8>, AetheriumSealevelError> {
    let binary = bs58::decode(base58)
        .into_vec()
        .map_err(AetheriumSealevelError::Decoding)?;
    Ok(binary)
}

pub fn decode_h256(base58: &str) -> Result<H256, AetheriumSealevelError> {
...
pub fn decode_h512(base58: &str) -> Result<H512, AetheriumSealevelError> {
    let binary = from_base58(base58)?;
...
pub fn decode_pubkey(address: &str) -> Result<Pubkey, AetheriumSealevelError> {
    Pubkey::from_str(address).map_err(Into::<AetheriumSealevelError>::into)
}
