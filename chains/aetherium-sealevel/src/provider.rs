...
use aetherium_core::{
    utils::to_atto, BlockInfo, ChainCommunicationError, ChainInfo, ChainResult, AetheriumChain,
    AetheriumDomain, AetheriumProvider, AetheriumProviderError, NativeToken, TxnInfo,
...
impl SealevelProvider {
    ...
    fn validate_transaction(hash: &H512, txn: &UiTransaction) -> ChainResult<()> {
        let received_signature = txn
            .signatures
            .first()
            .ok_or(AetheriumSealevelError::UnsignedTransaction(Box::new(*hash)))?;
        let received_hash = decode_h512(received_signature)?;

        if &received_hash != hash {
            Err(Into::<ChainCommunicationError>::into(
                AetheriumSealevelError::IncorrectTransaction(
                    Box::new(*hash),
    ...
    fn gas(meta: &UiTransactionStatusMeta) -> ChainResult<U256> {
        let OptionSerializer::Some(gas) = meta.compute_units_consumed else {
            Err(AetheriumSealevelError::EmptyComputeUnitsConsumed)?
        };
    ...
    fn meta(txn: &EncodedTransactionWithStatusMeta) -> ChainResult<&UiTransactionStatusMeta> {
        let meta = txn
            .meta
            .as_ref()
            .ok_or(AetheriumSealevelError::EmptyMetadata)?;
        Ok(meta)
    }
    ...
    async fn block_info_by_height(&self, slot: u64) -> Result<BlockInfo, ChainCommunicationError> {
        ...
        let block_time = confirmed_block
            .block_time
            .ok_or(AetheriumProviderError::CouldNotFindBlockByHeight(slot))?;
...
impl AetheriumChain for SealevelProvider {
...
#[async_trait]
impl AetheriumProvider for SealevelProvider {
