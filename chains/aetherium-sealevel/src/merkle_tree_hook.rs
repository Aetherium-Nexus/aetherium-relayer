...
use aetherium_core::{
    ...
    AetheriumChain, AetheriumMessage, Indexed, Indexer, LogMeta, MerkleTreeHook,
    MerkleTreeInsertion, ReorgPeriod, SequenceAwareIndexer,
};
use aetherium_sealevel_mailbox::accounts::OutboxAccount;
...
impl Indexer<MerkleTreeInsertion> for SealevelMerkleTreeHookIndexer {
    async fn fetch_logs_in_range(
        ...
        let messages = Indexer::<AetheriumMessage>::fetch_logs_in_range(&self.0, range).await?;
...
impl SequenceAwareIndexer<MerkleTreeInsertion> for SealevelMerkleTreeHookIndexer {
    async fn latest_sequence_count_and_tip(&self) -> ChainResult<(Option<u32>, u32)> {
        SequenceAwareIndexer::<AetheriumMessage>::latest_sequence_count_and_tip(&self.0).await
    }
}

fn message_to_merkle_tree_insertion(message: &AetheriumMessage) -> MerkleTreeInsertion {
