...
use aetherium_core::{ChainCommunicationError, ChainResult, U256};

use crate::{
    error::AetheriumSealevelError, priority_fee::PriorityFeeOracle,
...
impl SealevelRpcClient {
    ...
    pub async fn confirm_transaction_with_commitment(
        ...
        self.0
            ...
            .map(|ctx| ctx.value)
            .map_err(Box::new)
            .map_err(AetheriumSealevelError::ClientError)
            .map_err(Into::into)
    }
    ...
    pub async fn get_balance(&self, pubkey: &Pubkey) -> ChainResult<U256> {
        let balance = self
            ...
            .await
            .map_err(Box::new)
            .map_err(Into::<AetheriumSealevelError>::into)
