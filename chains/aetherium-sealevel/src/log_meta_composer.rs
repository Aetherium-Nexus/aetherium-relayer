...
use aetherium_core::{LogMeta, H512, U256};
...
impl LogMetaComposer {
    ...
    pub fn log_meta(
        ...
        pda_pubkey: &Pubkey,
        pda_slot: &Slot,
    ) -> Result<LogMeta, AetheriumSealevelError> {
        let block_hash = decode_h256(&block.blockhash)?;

        let transactions = block
            .transactions
            .ok_or(AetheriumSealevelError::NoTransactions(format!(
        ...
        if transaction_hashes.len() > 1 {
            Err(AetheriumSealevelError::TooManyTransactions(format!(
...
pub fn is_message_dispatch_instruction(instruction_data: &[u8]) -> bool {
    use aetherium_sealevel_mailbox::instruction::Instruction;
...
pub fn is_interchain_payment_instruction(instruction_data: &[u8]) -> bool {
    use aetherium_sealevel_igp::instruction::Instruction;
