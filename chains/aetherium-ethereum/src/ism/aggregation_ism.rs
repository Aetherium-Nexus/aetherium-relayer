...
use aetherium_core::{
    AggregationIsm, ChainResult, ContractLocator, AetheriumAbi, AetheriumChain, AetheriumContract,
    AetheriumDomain, AetheriumMessage, AetheriumProvider, RawAetheriumMessage, H256,
};
...
impl<M> AetheriumChain for EthereumAggregationIsm<M>
where
    M: Middleware + 'static,
{
...
impl<M> AetheriumContract for EthereumAggregationIsm<M>
where
    M: Middleware + 'static,
{
    ...
    M: Middleware + 'static,
{
    ...
    async fn modules_and_threshold(
        ...
        let (isms, threshold) = self
            .contract
            .modules_and_threshold(RawAetheriumMessage::from(message).to_vec().into())
            .call()
            .await?;
...
impl AetheriumAbi for EthereumAggregationIsmAbi {
