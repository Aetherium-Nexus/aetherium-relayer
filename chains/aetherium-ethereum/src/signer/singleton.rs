...
use aetherium_core::{
    AetheriumSigner, AetheriumSignerError, Signature as AetheriumSignature, H160, H256,
};

use crate::Signers;

/// A callback to send the result of a signing operation
type Callback = oneshot::Sender<Result<Signature, AetheriumSignerError>>;
...
/// A `AetheriumSigner` which grants access to a singleton signer via a channel.
#[derive(Clone)]
...
#[async_trait]
impl AetheriumSigner for SingletonSignerHandle {
    fn eth_address(&self) -> H160 {
        self.address
    }

    async fn sign_hash(&self, hash: &H256) -> Result<AetheriumSignature, AetheriumSignerError> {
...
impl From<SingletonSignerError> for AetheriumSignerError {
