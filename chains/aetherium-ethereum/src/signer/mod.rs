...
#[async_trait]
impl AetheriumSigner for Signers {
    ...
    async fn sign_hash(&self, hash: &H256) -> Result<AetheriumSignature, AetheriumSignerError> {
        let mut signature = Signer::sign_message(self, hash)
            .await
            .map_err(|err| AetheriumSignerError::from(Box::new(err) as Box<_>))?;
...
mod test {
    use aetherium_core::{
        Checkpoint, CheckpointWithMessageId, AetheriumSigner, AetheriumSignerExt, H256,
    };
