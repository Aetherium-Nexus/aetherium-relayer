...
use aetherium_core::{
    ...
    test_utils,
    utils::domain_hash,
    AetheriumMessage, H160, H256,
};

/// Output proof to /vector/message.json
#[test]
pub fn output_message() {
    let aetherium_message = AetheriumMessage {
        nonce: 0,
        version: 3,
        origin: 1000,
    ...
    let message_json = json!({
        "nonce": aetherium_message.nonce,
        "version": aetherium_message.version,
        "origin": aetherium_message.origin,
        "sender": aetherium_message.sender,
        "destination": aetherium_message.destination,
        "recipient": aetherium_message.recipient,
        "body": aetherium_message.body,
        "id": aetherium_message.id(),
    });
