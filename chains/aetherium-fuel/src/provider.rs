...
use aetherium_core::{
    h512_to_bytes, BlockInfo, ChainCommunicationError, ChainInfo, ChainResult, AetheriumChain,
    AetheriumDomain, AetheriumMessage, AetheriumProvider, AetheriumProviderError, Indexed, LogMeta,
...
impl FuelProvider {
    ...
    pub async fn new(domain: AetheriumDomain, conf: &ConnectionConf) -> Self {
        ...
        let full_tx_data = transaction_data
            ...
            .filter_map(|(tx_id, tx_data)| {
                ...
                if !receipt_log_data.is_empty() {
                    ...
                    let encoded_message = AetheriumMessage::from(receipt_log_data);
                    Some((tx_id, tx_data, encoded_message, log_index))
                } else {
                    None
                }
            })
            .collect::<Vec<(Bytes32, TransactionResponse, AetheriumMessage, U256)>>(); // Collect all Vec<u8> from each transaction into a Vec<Vec<u8>>

        let indexed_logs: Vec<(Indexed<AetheriumMessage>, LogMeta)> = full_tx_data
            .into_iter()
...
impl AetheriumChain for FuelProvider {
...
#[async_trait]
impl AetheriumProvider for FuelProvider {
    /// Used by scraper
    #[allow(clippy::clone_on_copy)] // TODO: `rustc` 1.80.1 clippy issue
    async fn get_block_by_height(&self, height: u64) -> ChainResult<BlockInfo> {
        let block_res = self
            ...
            .await
            .map_err(|e| AetheriumProviderError::CouldNotFindBlockByHeight(height))?;
        ...
        let block_info = match block_res {
            ...
            None => Err(AetheriumProviderError::CouldNotFindBlockByHeight(height))?,
        };
    ...
    /// Used by aetherium base metrics (scraper)
