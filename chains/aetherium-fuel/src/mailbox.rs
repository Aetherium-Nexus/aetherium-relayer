...
use aetherium_core::{
    utils::bytes_to_hex, ChainCommunicationError, ChainResult, ContractLocator, AetheriumAbi,
    AetheriumChain, AetheriumContract, AetheriumDomain, AetheriumMessage, AetheriumProvider,
    Indexed, Indexer, LogMeta, Mailbox, RawAetheriumMessage, ReorgPeriod, SequenceAwareIndexer,
...
impl AetheriumContract for FuelMailbox {
    fn address(&self) -> H256 {
        self.contract.contract_id().into_h256()
    }
}

impl AetheriumChain for FuelMailbox {
...
impl Mailbox for FuelMailbox {
    ...
    async fn process(
        ...
        let call_res = self
            ...
            .process(
                ...
                Bytes(RawAetheriumMessage::from(message)),
            )
    ...
    #[instrument(err, ret, skip(self), fields(aet_message=%message, metadata=%bytes_to_hex(metadata)))]
    #[allow(clippy::blocks_in_conditions)] // TODO: `rustc` 1.80.1 clippy issue
...
#[async_trait]
impl Indexer<AetheriumMessage> for FuelMailboxIndexer {
...
#[async_trait]
impl SequenceAwareIndexer<AetheriumMessage> for FuelMailboxIndexer {
...
impl AetheriumAbi for FuelMailboxAbi {
