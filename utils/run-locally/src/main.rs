...
//! Run this from the aetherium-relayer/rust directory using `cargo run -r -p
//! run-locally`.
//!
...
use aetherium_core::{PendingOperationStatus, ReorgEvent, ReprepareReason};
use logging::log;
...
fn main() -> ExitCode {
    ...
    let base_validator_env = common_agent_env
        ...
        .aet_env("CHAINS_TEST1_RPCCONSENSUSTYPE", "quorum")
        .aet_env(
            "CHAINS_TEST2_CUSTOMRPCURLS",
            "http://127.0.0.1:8545,http://127.0.0.1:8545,http://127.0.0.1:8545",
        )
        .aet_env("CHAINS_TEST2_RPCCONSENSUSTYPE", "fallback")
        .aet_env("CHAINS_TEST3_CUSTOMRPCURLS", "http://127.0.0.1:8545")
        .aet_env("CHAINS_TEST1_BLOCKS_REORGPERIOD", "0")
        .aet_env("CHAINS_TEST2_BLOCKS_REORGPERIOD", "0")
        .aet_env("CHAINS_TEST3_BLOCKS_REORGPERIOD", "0")
        .aet_env("INTERVAL", "5")
        .aet_env("CHECKPOINTSYNCER_TYPE", "localStorage");

    let validator_envs = (0..validator_count)
        .map(|i| {
            base_validator_env
                .clone()
                .aet_env("METRICSPORT", (9094 + i).to_string())
                .aet_env("DB", validator_dbs[i].to_str().unwrap())
                .aet_env("ORIGINCHAINNAME", validator_origin_chains[i])
                .aet_env("VALIDATOR_KEY", validator_keys[i])
                .aet_env(
    ...
    let scraper_env = common_agent_env
        ...
        .aet_env("CHAINS_TEST1_RPCCONSENSUSTYPE", "quorum")
        .aet_env("CHAINS_TEST1_CUSTOMRPCURLS", "http://127.0.0.1:8545")
        .aet_env("CHAINS_TEST2_RPCCONSENSUSTYPE", "quorum")
        .aet_env("CHAINS_TEST2_CUSTOMRPCURLS", "http://127.0.0.1:8545")
        .aet_env("CHAINS_TEST3_RPCCONSENSUSTYPE", "quorum")
        .aet_env("CHAINS_TEST3_CUSTOMRPCURLS", "http://127.0.0.1:8545")
        .aet_env("METRICSPORT", SCRAPER_METRICS_PORT)
        .aet_env(
            "DB",
            "postgresql://postgres:47221c18c610@localhost:5432/postgres",
        )
        .aet_env("CHAINSTOSCRAPE", "test1,test2,test3");
...
fn create_common_agent() -> Program {
    Program::default()
        ...
        .aet_env("LOG_FORMAT", "compact")
        .aet_env("LOG_LEVEL", "debug")
        .aet_env("CHAINS_TEST1_INDEX_CHUNK", "1")
        .aet_env("CHAINS_TEST2_INDEX_CHUNK", "1")
        .aet_env("CHAINS_TEST3_INDEX_CHUNK", "1")
}
...
fn create_relayer(rocks_db_dir: &TempDir) -> Program {
    ...
    common_agent_env
        ...
        .aet_env("CHAINS_TEST1_RPCCONSENSUSTYPE", "fallback")
        .aet_env(
        ...
        .aet_env("CHAINS_TEST1_MAXBATCHSIZE", "5")
        ...
        .aet_env("CHAINS_TEST2_MAXBATCHSIZE", "5")
        .aet_env("CHAINS_TEST3_CONNECTION_URL", "http://127.0.0.1:8545")
        .aet_env(
            "CHAINS_TEST3_BATCHCONTRACTADDRESS",
            multicall_address_string,
        )
        .aet_env("CHAINS_TEST3_MAXBATCHSIZE", "5")
        .aet_env("METRICSPORT", RELAYER_METRICS_PORT)
        .aet_env("DB", relayer_db.to_str().unwrap())
        .aet_env("CHAINS_TEST1_SIGNER_KEY", RELAYER_KEYS[0])
        .aet_env("CHAINS_TEST2_SIGNER_KEY", RELAYER_KEYS[1])
        .aet_env("RELAYCHAINS", "invalidchain,otherinvalid")
        .aet_env("ALLOWLOCALCHECKPOINTSYNCERS", "true")
        .aet_env(
...
fn relayer_reorg_handling_invariants_met() -> eyre::Result<bool> {
    let refused_messages = fetch_metric(
        ...
        "aetherium_submitter_queue_length",
        &HashMap::from([(
            "operation_status",
