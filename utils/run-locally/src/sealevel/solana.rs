...
const SOLANA_PROGRAM_LIBRARY_ARCHIVE: &str =
    "https://github.com/Aetherium-Nexus/solana-program-library/releases/download/2024-08-23/spl.tar.gz";
...
const SOLANA_AETHERIUM_PROGRAMS: &[&str] = &[
    "mailbox",
    "validator-announce",
    "ism/multisig-ism-message-id",
    "aetherium-sealevel-token",
    "aetherium-sealevel-token-native",
    "aetherium-sealevel-token-collateral",
    "aetherium-sealevel-igp",
];
...
pub fn build_solana_programs(solana_cli_tools_path: PathBuf) -> PathBuf {
    ...
    // build our programs
    for &path in SOLANA_AETHERIUM_PROGRAMS {
        build_sbf
            .clone()
    ...
    log!("All aetherium solana programs built successfully");
    out_path
}
...
pub fn start_solana_test_validator(
    ...
    log!("Deploying the aetherium programs to solana");
    ...
    log!("Local Solana chain started and aetherium programs deployed and initialized successfully");
...
pub fn initiate_solana_aetherium_transfer(
    solana_cli_tools_path: PathBuf,
...
fn sealevel_client(solana_cli_tools_path: &Path, solana_config_path: &Path) -> Program {
    ...
    Program::new(concat_path(
        &sealevel_path,
        format!("{}/aetherium-sealevel-client", SOLANA_AGENT_BIN_PATH),
    ))
