...
fn run_locally() {
    ...
    let relayer_env = common_agent_env
        ...
        .aet_env("CHAINS_SEALEVELTEST1_SIGNER_KEY", RELAYER_KEYS[0])
        .aet_env("CHAINS_SEALEVELTEST2_SIGNER_KEY", RELAYER_KEYS[1])
    ...
    let scraper_env = common_agent_env
        ...
        .aet_env("CHAINSTOSCRAPE", "sealeveltest1,sealeveltest2");
    ...
    let (solana_programs_path, aetherium_solana_programs_path) = {
    ...
    log!("Building aetherium-sealevel-client...");
    Program::new("cargo")
        .working_dir(&sealevel_path)
        .cmd("build")
        .arg("bin", "aetherium-sealevel-client")
        .filter_logs(|l| !l.contains("workspace-inheritance"))
        .run()
        .join();
    ...
    let (solana_cli_tools_path, solana_config_path) = {
        ...
        let start_solana_validator = start_solana_test_validator(
            ...
            aetherium_solana_programs_path.clone(),
    ...
    for _i in 0..(SOL_MESSAGES_EXPECTED / 2) {
        initiate_solana_aetherium_transfer(
