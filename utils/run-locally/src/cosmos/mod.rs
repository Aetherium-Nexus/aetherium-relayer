...
use aetherium_cosmos::RawCosmosAmount;
use aetherium_cosmwasm_interface::types::bech32_decode;
...
use self::deploy::deploy_cw_aetherium;
...
const CW_AETHERIUM_GIT: &str = "https://github.com/Aetherium-Nexus/cosmwasm";
const CW_AETHERIUM_VERSION: &str = "v0.0.6";
...
pub fn install_codes(dir: Option<PathBuf>, local: bool) -> BTreeMap<String, PathBuf> {
    ...
    if !local {
        ...
        log!(
            "Downloading {} @ {}",
            CW_AETHERIUM_GIT,
            CW_AETHERIUM_VERSION
        );
        let uri =
            format!("{CW_AETHERIUM_GIT}/releases/download/{CW_AETHERIUM_VERSION}/{release_comp}");
        download(release_comp, &uri, dir_path_str);

        log!("Uncompressing {} release", CW_AETHERIUM_GIT);
        unzip(release_comp, dir_path_str);
    }

    log!("Installing {} in Path: {:?}", CW_AETHERIUM_GIT, dir_path);
...
pub struct CosmosAetheriumStack {
...
impl Drop for CosmosAetheriumStack {
    fn drop(&mut self) {
...
fn launch_cosmos_validator(
    ...
    let validator = Program::default()
        ...
        .aet_env("CHECKPOINTSYNCER_PATH", checkpoint_path.to_str().unwrap())
        .aet_env("CHECKPOINTSYNCER_TYPE", "localStorage")
        .aet_env("ORIGINCHAINNAME", agent_config.name)
        .aet_env("DB", validator_base_db.to_str().unwrap())
        .aet_env("METRICSPORT", agent_config.metrics_port.to_string())
        .aet_env("VALIDATOR_SIGNER_TYPE", agent_config.signer.typ)
        .aet_env("VALIDATOR_KEY", agent_config.signer.key.clone())
        .aet_env("VALIDATOR_PREFIX", "osmo")
        .aet_env("SIGNER_SIGNER_TYPE", "hexKey")
        .aet_env("SIGNER_KEY", agent_config.signer.key)
        .aet_env("TRACING_LEVEL", if debug { "debug" } else { "info" })
        .spawn("VAL", None);
...
fn launch_cosmos_relayer(
    ...
    let relayer = Program::default()
        ...
        .env("RUST_BACKTRACE", "1")
        .aet_env("RELAYCHAINS", relay_chains.join(","))
        .aet_env("DB", relayer_base.as_ref().to_str().unwrap())
        .aet_env("ALLOWLOCALCHECKPOINTSYNCERS", "true")
        .aet_env("CHAINS_COSMOSTEST99990_MAXBATCHSIZE", "5")
        .aet_env("CHAINS_COSMOSTEST99991_MAXBATCHSIZE", "5")
        .aet_env("TRACING_LEVEL", if debug { "debug" } else { "info" })
        .aet_env("GASPAYMENTENFORCEMENT", "[{\"type\": \"none\"}]")
        .aet_env("METRICSPORT", metrics.to_string())
        .spawn("RLY", None);
...
fn launch_cosmos_scraper(
    ...
    let scraper = Program::default()
        ...
        .env("CONFIG_FILES", agent_config_path)
        .env("RUST_BACKTRACE", "1")
        .aet_env("CHAINSTOSCRAPE", chains.join(","))
        .aet_env(
            "DB",
...
const ENV_CW_AETHERIUM_PATH_KEY: &str = "E2E_CW_AETHERIUM_PATH";
...
fn run_locally() {
    ...
    let code_src = Some(
        env::var(ENV_CW_AETHERIUM_PATH_KEY)
            .as_ref()
    ...
    let nodes = nodes
        ...
        .map(|(launch_resp, chain_id, metrics_port, domain)| {
            let deployments = deploy_cw_aetherium(
    ...
    let _stack = CosmosAetheriumStack {
